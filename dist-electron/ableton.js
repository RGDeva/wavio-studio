...
exports.registerAbletonHandlers = registerAbletonHandlers;
...
function registerAbletonHandlers() {
    if (_registered)
        return;
    _registered = true;
    electron_1.ipcMain.handle('ableton:select-folder', async () => {
    ...
    electron_1.ipcMain.handle('ableton:sync-to-cloud', async (_e, snapshot, authToken) => {
        if (!authToken)
            return { error: 'Not authenticated with Wavi' };
        try {
...
async function scanAbletonFolder(folderPath) {
    const detectedFiles = [];
...
function detectDawSource(files) {
...
async function parseAbletonLiveSet(alsPath) {
    try {
...
function getFileRole(ext) {
    ...
    if (FL_EXTS.has(ext) || LOGIC_EXTS.has(ext) || PT_EXTS.has(ext))
        return 'project';
