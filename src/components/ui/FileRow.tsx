...
export function FileRow({ fileName, relPath, size, missing, synced, included, checksum, onOpen, onReveal }: FileRowProps) {
  ...
  return (
    <div className="group flex items-start gap-2 px-3 py-2.5 hover:bg-white/[0.03] transition-colors border-b border-border-subtle last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          ...
          {missing && <span className="text-[9px] text-destructive bg-destructive/10 rounded px-1">missing</span>}
          {included && <span className="text-[9px] text-success/80 bg-success/10 rounded px-1">in package</span>}
        </div>
        ...
        <div className="flex items-center gap-2 mt-1">
          {synced
            ? <span className="flex items-center gap-0.5 text-[9px] text-primary/70"><Cloud className="w-2.5 h-2.5" /> synced</span>
          ...
          {checksum && (
            <button
              ...
              title="Copy checksum"
              className={cn('flex items-center gap-0.5 text-[9px] text-white/20 hover:text-white/50',
      ...
      {!missing && (onOpen || onReveal) && (
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity flex-shrink-0">
          {onOpen && (
            <button type="button" onClick={onOpen} title="Open" className="p-1 rounded hover:bg-white/10 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
              <ExternalLink className="w-3 h-3 text-white/40" />
            </button>
          )}
          {onReveal && (
            <button type="button" onClick={onReveal} title="Reveal in Finder" className="p-1 rounded hover:bg-white/10 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
