...
export function CopilotPanel() {
  ...
  return (
    <div
      className="flex flex-col h-screen bg-[#0a0a0a]/95 backdrop-blur-xl text-white rounded-2xl border border-hairline-strong shadow-2xl overflow-hidden"
      style={{ fontFamily: 'Inter, sans-serif' }}
    >
      {/* Header */}
      <div className="flex items-center gap-2 px-4 py-3 border-b border-hairline-strong flex-shrink-0">
        <div className="w-5 h-5 rounded-full bg-primary/20 border border-primary/40 flex items-center justify-center">
          <Zap className="w-3 h-3 text-primary" />
        </div>
        <span className="text-sm font-semibold text-fg-secondary">Wavi Copilot</span>
        <div className="flex items-center gap-1.5 ml-auto">
          {/* Memory badge */}
          {memoryCount > 0 && (
            <div className="flex items-center gap-1 bg-accent/15 border border-accent/20 rounded-full px-1.5 py-0.5" title={`${memoryCount} memories stored`}>
              <Brain className="w-2.5 h-2.5 text-accent" />
              <span className="text-meta text-accent">{memoryCount}</span>
            </div>
          )}
          {/* Ask / Act toggle */}
          <button
            onClick={() => setMode(m => m === 'ask' ? 'act' : 'ask')}
            className={`flex items-center gap-1 rounded-full px-2 py-0.5 text-meta transition-all border ${
              mode === 'act'
                ? 'bg-primary/20 border-primary/30 text-primary'
                : 'bg-layer-2 border-hairline-strong text-fg-quaternary hover:text-fg-tertiary'
            }`}
          ...
          <button
            ...
            className="w-6 h-6 rounded-full flex items-center justify-center text-fg-quaternary hover:text-fg-secondary hover:bg-layer-3 transition-all"
          >
      ...
      <div className="flex-shrink-0 border-b border-hairline">
        <button
          ...
          className="w-full flex items-center justify-between px-4 py-2 text-meta text-fg-quaternary hover:text-fg-tertiary transition-colors"
        >
          <div className="flex items-center gap-1.5">
            <MessageSquare className="w-3 h-3" />
            <span className="uppercase tracking-wider">Project Context</span>
            {context?.projectName && (
              <span className="text-primary/60 ml-1">{context.projectName}</span>
            )}
            {contextLoading && <Loader2 className="w-3 h-3 text-primary/60 animate-spin ml-1" />}
          </div>
      ...
      >
        {messages.length === 0 && (
          <div className="text-center py-4">
            <p className="text-xs text-fg-quaternary mb-4">Ask Wavi anything about your project</p>
            <div className="space-y-1.5">
              {SUGGESTED_PROMPTS.map((p) => (
                <button
                  key={p}
                  onClick={() => send(p)}
                  className="block w-full text-left text-meta text-fg-quaternary hover:text-fg-tertiary bg-layer-1 hover:bg-layer-3 border border-hairline hover:border-hairline-strong rounded-lg px-3 py-2 transition-all"
                >
                  {p}
                </button>
              ))}
      ...
      {mode === 'act' && (
        <div className="flex-shrink-0 border-t border-hairline px-3 py-2">
          <p className="text-meta text-fg-quaternary uppercase tracking-wider mb-1.5">Quick Actions</p>
          <div className="flex flex-wrap gap-1">
            {ACT_TOOLS.map(t => (
              <button key={t.label} onClick={() => send(t.prompt)} disabled={sending}
                className="px-2.5 py-1 rounded-lg text-meta bg-layer-2 hover:bg-primary/15 border border-hairline-strong hover:border-primary/25 text-fg-tertiary hover:text-primary transition-all disabled:opacity-40">
                {t.label}
              </button>
            ))}
          </div>
      ...
      {/* Input */}
      <div className="flex-shrink-0 border-t border-hairline-strong px-3 py-3">
        <div className="flex gap-2 items-end">
          <textarea
            ...
            disabled={sending}
            rows={1}
            className="flex-1 bg-layer-3 border border-hairline-strong rounded-xl px-3 py-2 text-xs text-fg-secondary placeholder-white/25 resize-none focus:outline-none focus:border-primary/40 transition-colors disabled:opacity-50"
          ...
          <button
            ...
            className="w-8 h-8 rounded-xl bg-primary hover:bg-primary disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center transition-all flex-shrink-0"
          >
            <Send className="w-3.5 h-3.5 text-black" />
          </button>
        </div>
        <p className="text-meta text-fg-quaternary mt-1.5 text-center">
