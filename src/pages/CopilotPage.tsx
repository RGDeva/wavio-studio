  ...
  return (
    <div className="h-full flex">
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex-shrink-0 flex items-center justify-between px-6 py-4 border-b border-hairline">
          <div className="flex items-center gap-2">
            ...
            <label className="ml-2 flex items-center gap-1 px-2 py-0.5 rounded-md text-meta border border-hairline-strong text-fg-quaternary" title="Tools act on this project">
              ...
              <select
                ...
                className="bg-transparent text-fg-tertiary text-meta focus:outline-none max-w-[160px]"
              >
          ...
          <div className="flex items-center gap-2">
            {discovering ? (
              <div className="flex items-center gap-2">
                <span className="text-meta text-fg-quaternary max-w-[200px] truncate">{progressLabel}</span>
                <button
                  onClick={handleCancelDiscover}
                  className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs border border-hairline-strong text-destructive/70 hover:text-destructive transition-colors"
                >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs border border-hairline-strong text-fg-quaternary hover:text-fg-secondary hover:border-hairline-focus transition-all"
              >
            ...
            <button onClick={() => setShowMemory(s => !s)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs border transition-all ${showMemory?'bg-primary/20 text-primary border-primary/30':'text-fg-quaternary border-hairline-strong hover:text-fg-secondary'}`}>
              <BookOpen className="w-3.5 h-3.5" />Memory
            </button>
            <button onClick={() => setMessages([])} className="text-fg-quaternary hover:text-fg-tertiary transition-colors p-1.5 rounded-lg hover:bg-layer-2">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {messages.length === 0 && (
            <div className="py-6">
              <div className="text-center mb-5">
                <Zap className="w-8 h-8 text-primary/40 mx-auto mb-2" />
                <p className="text-fg-quaternary text-sm font-medium">Wavi Copilot</p>
                <p className="text-fg-quaternary text-xs mt-1">{SEARCH_NOTE}</p>
              </div>
              <div className="grid grid-cols-1 gap-1.5">
                {PROMPTS.map(p => (
                  <button key={p} onClick={() => send(p)}
                    className="w-full text-left px-4 py-2.5 rounded-xl bg-layer-1 hover:bg-layer-3 border border-hairline hover:border-primary/20 text-sm text-fg-tertiary hover:text-fg-secondary transition-all">
                    {p}
                  </button>
                ))}
              </div>
              <p className="text-center text-meta text-fg-quaternary mt-4">
                Hit <span className="font-mono bg-layer-2 px-1 rounded">Discover</span> to scan your Mac for audio files first
              </p>
            </div>
          )}
          {messages.map(m => (
            <div key={m.id} className={`flex ${m.role==='user'?'justify-end':'justify-start'}`}>
              <div className={`max-w-[80%] px-4 py-3 rounded-2xl text-sm whitespace-pre-wrap ${m.role==='user'?'bg-primary/20 text-white rounded-br-sm':'bg-layer-2 text-fg rounded-bl-sm'}`}>
                {m.content}
              </div>
            </div>
          ))}
          {loading && (
            <div className="flex justify-start">
              <div className="bg-layer-2 px-4 py-3 rounded-2xl rounded-bl-sm flex gap-1">
                {[0,1,2].map(i => <span key={i} className="w-1.5 h-1.5 bg-layer-10 rounded-full animate-bounce" style={{animationDelay:`${i*0.15}s`}} />)}
              </div>
            </div>
          )}
          {pendingConfirm && (
            <div className="mx-1 my-2 bg-warning/5 border border-warning/30 rounded-xl p-4 space-y-3" role="alertdialog" aria-label="Confirm Copilot action">
              <p className="text-xs font-semibold text-warning">
                Copilot wants to run: <span className="font-mono">{pendingConfirm.tool.replace(/_/g, ' ')}</span>
              </p>
              <p className="text-xs text-fg-secondary leading-relaxed">{pendingConfirm.summary}</p>
              <div className="text-meta text-fg-quaternary space-y-0.5">
              ...
              <div className="flex items-center justify-end gap-2">
                <button onClick={handleCancelConfirm} disabled={confirmBusy}
                  className="px-3 py-1.5 rounded-lg text-xs font-medium text-fg-tertiary hover:text-fg border border-hairline-strong">
                  Cancel
                </button>
                <button onClick={handleConfirm} disabled={confirmBusy}
                  className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-warning/20 hover:bg-warning/30 text-warning disabled:opacity-40">
        ...
        <div className="flex-shrink-0 px-6 pb-6 pt-2">
          <div className="flex gap-2 items-end bg-layer-2 border border-hairline-strong rounded-2xl px-4 py-3 focus-within:border-primary/30 transition-all">
            ...
            <button onClick={() => send()} disabled={!input.trim() || loading}
              className="flex-shrink-0 w-8 h-8 bg-primary hover:bg-primary disabled:opacity-30 rounded-xl flex items-center justify-center transition-all">
              <Send className="w-3.5 h-3.5 text-black" />
            </button>
          </div>
          <p className="text-meta text-fg-quaternary mt-1.5 text-center">⌘⇧W for overlay · Enter to send · Shift+Enter for newline</p>
        </div>
      </div>

      {showMemory && (
        <div className="w-72 flex-shrink-0 border-l border-hairline flex flex-col">
          <div className="flex-shrink-0 px-4 py-4 border-b border-hairline">
            <h2 className="text-sm font-semibold text-white">Memory</h2>
            <p className="text-xs text-fg-quaternary mt-0.5">Copilot remembers these</p>
          </div>
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
            {memories.length === 0 && <p className="text-xs text-fg-quaternary py-4 text-center">No memories yet</p>}
            {memories.map(m => (
              <div key={m.key} className="flex items-start gap-2 group">
                <div className="flex-1 min-w-0 bg-layer-1 rounded-lg px-3 py-2">
                  <p className="text-meta text-fg-quaternary truncate">{m.key}</p>
                  <p className="text-xs text-fg-secondary mt-0.5 line-clamp-2">{m.value}</p>
                </div>
                <button onClick={() => deleteMemory(m.key)} className="flex-shrink-0 opacity-0 group-hover:opacity-100 p-1 text-fg-quaternary hover:text-destructive transition-all">
                  <Trash2 className="w-3 h-3" />
                </button>
              </div>
            ))}
          </div>
          <div className="flex-shrink-0 px-4 py-3 border-t border-hairline space-y-2">
            <input value={newKey} onChange={e => setNewKey(e.target.value)} placeholder="Key"
              className="w-full bg-layer-2 border border-hairline-strong rounded-lg px-3 py-1.5 text-xs text-white placeholder-white/25 focus:outline-none focus:border-primary/30" />
            <input value={newVal} onChange={e => setNewVal(e.target.value)} placeholder="Value"
              className="w-full bg-layer-2 border border-hairline-strong rounded-lg px-3 py-1.5 text-xs text-white placeholder-white/25 focus:outline-none focus:border-primary/30" />
            <button onClick={saveMemory} className="w-full flex items-center justify-center gap-1.5 py-1.5 bg-layer-2 hover:bg-layer-3 border border-hairline-strong rounded-lg text-xs text-fg-tertiary hover:text-white transition-all">
