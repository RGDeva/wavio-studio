...
function parseWavHeader(buf: Buffer): WavInfo | null {
  try {
    ...
    if (audioFormat !== 1 || !sampleRate || !channels || !dataOffset) return null;
...
function parseAiffHeader(buf: Buffer): WavInfo | null {
  try {
    if (buf.toString('ascii', 0, 4) !== 'FORM') return null;
    const formType = buf.toString('ascii', 8, 12);
    if (formType !== 'AIFF') return null; // skip AIFF-C
    ...
    if (!sampleRate || !channels || !dataOffset) return null;
...
function extractSamples(buf: Buffer, info: WavInfo, maxSamples = 44100 * 30): Float32Array {
...
function estimateBpm(samples: Float32Array, originalRate: number): number | null {
  // 1. Downsample
  ...
  const env: number[] = new Array(ds.length).fill(0);
...
export async function detectBpm(filePath: string): Promise<number | null> {
  return new Promise((resolve) => {
    try {
      ...
      const size = Math.min(fs.fstatSync(fd).size, 10 * 1024 * 1024);
      ...
      if (samples.length < info.sampleRate * 2) return resolve(null); // need ≥2s
