...
export type FileRole = 'stem' | 'mix' | 'master' | 'reference' | 'sample' | 'unknown';
...
export function classifyFile(fileName: string): FileRole {
  ...
  if (MASTER_PATTERNS.some(r => r.test(name))) return 'master';
  if (MIX_PATTERNS.some(r => r.test(name))) return 'mix';
  if (STEM_PATTERNS.some(r => r.test(name))) return 'stem';
  if (REFERENCE_PATTERNS.some(r => r.test(name))) return 'reference';
  if (SAMPLE_PATTERNS.some(r => r.test(name))) return 'sample';
