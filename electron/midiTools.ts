...
function writeMidiFile(notes: MidiNote[], tempo: number, ticksPerBeat: number): Buffer {
...
function noteToMidi(key: string, octave: number): number {
  return (octave + 1) * 12 + (NOTE_MAP[key] ?? 0);
}

function scaleNotes(key: string, scale: string, octave = 4): number[] {
...
function getMidiOutputDir(projectFolder: string | null): string {
  const base = projectFolder
    ?? path.join(app.getPath('music'), 'Wavi', 'MIDI');
  const dir = path.join(base, 'Wavi_MIDI');
...
function saveFile(dir: string, filename: string, buffer: Buffer): string {
...
export function generateMidiMelody(params: {
  key?: string;
  scale?: string;
  tempo?: number;
  bars?: number;
...
export function generateChordProgression(params: {
  key?: string;
  scale?: string;
  progression?: string;
  tempo?: number;
  ...
  try {
    ...
    const chordQuality = (degree: string): number[] => {
...
export function generateDrumPattern(params: {
  pattern_type?: string;
  tempo?: number;
  bars?: number;
  projectFolder?: string | null;
  ...
  try {
    ...
    type DrumGrid = Record<number, number[]>; // pitch → step indices (0-15 per bar)
    const PATTERNS: Record<string, DrumGrid> = {
      trap: {
        [KICK]:   [0, 6, 10, 14],
        [SNARE]:  [4, 12],
...
export function revealProjectFolder(projectFolder: string | null): ToolResult {
  const dir = projectFolder ?? path.join(app.getPath('music'), 'Wavi', 'MIDI', 'Wavi_MIDI');
  try {
    fs.mkdirSync(dir, { recursive: true });
...
export function explainImportToFlStudio(): ToolResult {
  return {
    status: 'done',
...
export function summarizeProjectContext(context: {
  ...
  cloudVersions?: Array<{ versionNumber: number; syncedAt: string }>;
}): ToolResult {
  const lines: string[] = [];
