...
function applyTruncatedHashMigration(db: any) {
  db.exec(`
...
maybeDescribe('SQLite integration (real better-sqlite3, arm64 Node binary)', () => {
  ...
  it('initializes WAL mode on file-based database', () => {
  ...
  it('creates all required tables', () => {
    ...
    expect(tables).toContain('bounce_candidates');
  });

  it('schema migration is idempotent (CREATE IF NOT EXISTS)', () => {
    // Running the schema again on an existing DB must not throw
    expect(() => buildSchema(Database)).not.toThrow();
  ...
  it('nulls out 16-char hex truncated checksums in files table', () => {
    const id = 'f1';
    db.prepare(`INSERT INTO files (id, file_path, file_name, file_type, created_at, modified_at, checksum)
                VALUES (?, '/x', 'x.wav', 'wav', '2026-01-01', '2026-01-01', ?)`
    ).run(id, 'deadbeefcafe1234'); // 16-char hex

    applyTruncatedHashMigration(db);
    const row = db.prepare('SELECT checksum FROM files WHERE id = ?').get(id) as any;
  ...
  it('preserves 64-char full SHA-256 checksums', () => {
    ...
    applyTruncatedHashMigration(db);
    const row = db.prepare('SELECT checksum FROM files WHERE id = ?').get('f2') as any;
    expect(row.checksum).toBe(full);
  });

  it('does not null non-hex 16-char strings (GLOB guard)', () => {
    db.prepare(`INSERT INTO files (id, file_path, file_name, file_type, created_at, modified_at, checksum)
                VALUES (?, '/x', 'x.wav', 'wav', '2026-01-01', '2026-01-01', ?)`
    ).run('f3', 'MyPassphrase1234'); // 16 chars but has uppercase → not hex

    applyTruncatedHashMigration(db);
    const row = db.prepare('SELECT checksum FROM files WHERE id = ?').get('f3') as any;
    expect(row.checksum).toBe('MyPassphrase1234');
  });
  ...
  it('applies to bounce_candidates and versions tables too', () => {
    ...
    const v = db.prepare('SELECT checksum FROM versions WHERE id = ?').get('v1') as any;
    const b = db.prepare('SELECT checksum FROM bounce_candidates WHERE id = ?').get('b1') as any;
    expect(v.checksum).toBeNull();
    expect(b.checksum).toBeNull();
  });
  ...
  it('returns pending items with null next_retry_at immediately', () => {
    ...
    expect((items[0] as any).id).toBe('q1');
  });
  ...
  it('returns retrying items whose next_retry_at has passed', () => {
    ...
    const items = getPendingSyncItems(db, now);
    expect(items.length).toBe(1);
    expect((items[0] as any).id).toBe('q3');
  });
  ...
  it('persists and retrieves cloud_asset_id on files row', () => {
    ...
    const row = db.prepare('SELECT cloud_asset_id FROM files WHERE id = ?').get('f10') as any;
    expect(row.cloud_asset_id).toBe(assetId);
  });

  it('cloud_asset_id survives a simulated restart (close + reopen with file)', () => {
    ...
    const row = db3.prepare('SELECT cloud_asset_id FROM files WHERE id = ?').get('f11') as any;
    db3.close();
    expect(row.cloud_asset_id).toBe('restart-test-id');
  });
  ...
  it('persists cloud_version_id on project row', () => {
    ...
    const row = db.prepare('SELECT cloud_id, cloud_version_id FROM projects WHERE id = ?').get('p10') as any;
    expect(row.cloud_id).toBe('cloud-proj-123');
    expect(row.cloud_version_id).toBe('cloud-ver-456');
  });
  ...
  it('retry resets status to pending without creating duplicate rows', () => {
    ...
    const rows = db.prepare('SELECT * FROM sync_queue WHERE id = ?').all('q10') as any[];
