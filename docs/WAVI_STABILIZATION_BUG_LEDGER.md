...
| WAVI-P0-C | P0 | Entitlements | Founder has ZERO `user_plans` rows → Free fallback (26 files/500 MB); admin gating is an email allowlist in `api/_lib/entitlements.ts`, not durable server state | `fix/founder-entitlements` (WS-022, worker in progress) | per WS-022 packet | in progress; migration requires founder approval |
