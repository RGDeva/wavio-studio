...
|---|---|---|---|
| **ENV-1** | **An endpoint-security agent quarantines Electron at EXEC time.** Sharpened 2026-09-09 — this is not a cleanup job and not a broken download. Electron\u2019s own installer restores a complete bundle (252 files, 227MB, all Frameworks), and it **survives untouched for 12+ seconds** while idle. The moment it is executed: **SIGKILL (exit 137) and the bundle drops to 0 files** \u2014 measured 252 files at 15:20:49 \u2192 exec \u2192 0 files at 15:20:55. Reproduced with a hand-extracted copy AND with the official `install.js` output, and with the agent\u2019s own Bash sandbox disabled. So it is host security tooling, not agent tooling, not packaging. | host / IT | **Actionable fix: allowlist Electron in the endpoint-security product** (the repo\u2019s `node_modules/electron`, and any packaged `Wavi Studio.app`). Until then every Electron-dependent gate is unrunnable here. **Do not attempt to bypass** \u2014 do not re-sign, relocate, or otherwise evade the agent. |
