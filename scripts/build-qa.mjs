#!/usr/bin/env node
/**
 * Safe QA build wrapper — prevents electron-builder extraMetadata from
...
try {
  execSync(
    'npm run build && CSC_IDENTITY_AUTO_DISCOVERY=false npx electron-builder --mac dir --arm64 -c electron-builder.qa.json',
  ...
  if (restoredDigest !== originalDigest) {
    console.error('[build-qa] FATAL: package.json could not be restored — check file permissions');
    process.exit(1);
  }
  console.log('[build-qa] package.json restored ✓');
}
